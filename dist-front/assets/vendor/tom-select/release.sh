...
# remove contents of dist-front folder
if ! rm -r dist-front/*; then
	echo '/dist-front not emptied... cannot create release'
	exit
fi


# copy /build to /dist-front
if ! cp -r build/* dist-front; then
	echo '/build not copied to /dist-front... cannot create release'
	exit
fi
