...
gulp.task('js-build', function () {
	return gulp.src('./src/js/hs-toggle-state.js')
		.pipe(webpackStream({
			mode: 'development',
			devtool: 'source-map',
			output: {
				library: 'HSToggleState',
				libraryTarget: 'umd',
				libraryExport: 'default',
				filename: 'hs-toggle-state.min.js',
			},
			optimization: {
				minimize: true
			},
...
gulp.task('main-watch', function () {
	...
	gulp.watch('./src/js/hs-toggle-state.js', gulp.series('js-build'));
});
