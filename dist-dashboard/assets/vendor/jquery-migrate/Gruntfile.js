...
module.exports = function( grunt ) {
	...
	const karmaFilesExceptJQuery = [
		...
		{ pattern: "dist-front-dashboard/jquery-migrate.js", included: false, served: true },
	...
	grunt.initConfig( {
		...
		compare_size: {
			files: [ "dist-front-dashboard/jquery-migrate.js", "dist/jquery-migrate.min.js" ],
			options: {
				compress: {
		...
		concat: {
			...
			dist: {
				src: "<%= files %>",
				dest: "dist-front-dashboard/<%= pkg.name %>.js"
			}
		},
		build: {
			all: {
				src: "src/migrate.js",
				dest: "dist-front-dashboard/jquery-migrate.js"
			}
		},
		qunit: {
		...
		eslint: {
			...
			dist: {
				src: "dist-front-dashboard/jquery-migrate.js"
			},
			dev: {
				src: [
					"Gruntfile.js",
		...
		uglify: {
			all: {
				files: {
					"dist/jquery-migrate.min.js":
						[ "src/migratemute.js", "dist-front-dashboard/jquery-migrate.js" ]
				},
				options: {
					preserveComments: false,
					sourceMap: true,
					sourceMapName: "dist-front-dashboard/jquery-migrate.min.map",
					report: "min",
					output: {
						"ascii_only": true,
	...
	grunt.registerTask( "lint", [
		...
		// would run the dist-front-dashboard target first which would point to errors in the built
		// file, making it harder to fix them. We want to check the built file only
		// if we already know the source files pass the linter.
		"eslint:dev",
		"eslint:dist-front-dashboard"
	] );
